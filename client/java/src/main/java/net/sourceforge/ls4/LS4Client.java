...
public class LS4Client {
	private EventLoop loop;
	private Client client;
	private LS4ClientInterface proxy;

	public LS4Client(String host, int port) throws UnknownHostException {
		this(new InetSocketAddress(host, port));
	}

	public LS4Client(InetSocketAddress address) {
		this.loop = EventLoop.start();
		this.client = new Client(address, loop);
		this.proxy = client.proxy(LS4ClientInterface.class);
	}
